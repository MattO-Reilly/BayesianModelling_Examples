...
Complete Assignment 4 using this R markdown file, building a report on each of the statistical analyses. Submit your final report via TurnItIn to Blackboard **in Microsoft Word format**. This Assignment counts as 10% of the module grade.

## Question 1
Let's see if the weather is any better. In this question, you'll compare the probability of rain now to your estimate from Assignment 3. We'll treat each day as independent and suggest that $X$, the number of days it rains in Galway, follows a Binomial distribution, i.e. $X \sim Bin(n,\theta)$.

(a) Choose a continuous prior distribution for the probability $\theta$, and justify your choice. Use the same prior for both groups so that any observed differences can be attributed to the data. Summarise this prior using an appropriate plot. [If you are still happy with the prior from Assignment 3 then feel free to use it again here.] (**3 marks**)
```{r}
...
(b) Once again, collect data on rain in Galway over the course of several days. Let $n_2$ be the new total number of days on which you collect data, and $x_2$ be the new total number of days with rain. Let the old data from Assignment 3 involve $x_1$ days of rain out of $n_1$ total days. Create a vector of data parameters for each group in `R` and use this to update your prior distribution into two posteriors, $\theta_1 = \theta|X_1$ and $\theta_2 = \theta|X_2$. Plot and interpret the posteriors. (**12 marks**)
```{r}
n_1 = 11
x_1 = 7
data1 <- c(7,4)
...
(c) Simulate data from each posterior and use this **to obtain and plot** a posterior distribution for the difference $(\theta_1-\theta_2)|X$. Interpret the distribution of the difference. (**8 marks**)
```{r}
...
(d) Calculate and interpret a 95% credible interval for $(\theta_1-\theta_2)|X$ (**8 marks**)
```{r}
...
(e) What is the probability that $\theta_1>\theta_2$, i.e. that the probability of rain has decreased since Assignment 3? (**4 marks**)
```{r}
sum(simDiff>0)/length(simDiff)
```
The probability rain has decreased since Assignment 1 is `r sum(simDiff>0)/length(simDiff)`
...
(a) Choose a Normal prior distribution for the mean salary $\theta\sim N(\mu_0,\sigma_0^2)$ (**in units of €1000**). You can use the same prior for both Universities so that any differences we observe can be solely attributed to the data. (**3 marks**)
```{r}
mu0 <- 130
sd0 <- 10
```

You now observe the following data of salaries (**in units of €1000**) from the boards of Universities A, B and C. The `Board` data are contained in the `BSDA` package, which you'll need to install before continuing.
...
(b) Summarise the data for each group and use these summary statistics to update your prior distribution into three posteriors: $\theta_A = \theta|X_A$, $\theta_B = \theta|X_B$ and $\theta_C = \theta|X_C$. Plot the prior and three posterior distributions together and interpret this plot. (**10 marks**)
```{r}
...
(c) Use the posterior parameters together with the properties of the Normal distribution to work out both $(\theta_A-\theta_B)|X$ and $(\theta_A-\theta_C)|X$. Plot the two resulting posterior distributions of the difference in means, and interpret these densities. (**8 marks**)
```{r}
meanAB <- meanA - meanB
meanAC <- meanA - meanC
meanAB
...
(d) Calculate, plot and interpret 95% credible intervals for $(\theta_A-\theta_B)|X$ and $(\theta_A-\theta_C)|X$ separately (**8 marks**)
```{r}
...
(e) Are there any issues with the data? Test whether resolving these issues alters your inferences from part **d**. (**6 marks**)
```{r}
...
(a) Simulate $\theta$ values from a suitable Uniform prior and simulate a count of insurance claims for each $\theta$. Combine these simulated data into a data frame and plot the resulting distributions of $\theta$ for each simulated count of insurance claims. [Hint: the `geom_density_ridges` function in the `ggridges` package might be useful] (**8 marks**)
```{r}
...
(b) After a month, you observe 29 insurance claims. Use grid approximation to calculate a posterior distribution for $\theta|X$. Plot and interpret this posterior distribution. (**10 marks**)
```{r}
...
(c) Calculate, plot and interpret a 95% credible interval for the mean rate of claims per month. [Hint: `geom_vline`] (**8 marks**)
```{r}
...
(d) What is the probability that the mean number of claims is over 40 per month, i.e. $P(\theta>40)$? (**4 marks**)
```{r}
