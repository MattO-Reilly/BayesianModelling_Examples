...
```{r}
studentdata %>% summarise(mean = mean(studentdata$G3), sd = sd(studentdata$G3))
```
...
# Density plots of the prior_m samples 
ggplot(samples, aes(x = prior_m)) + 
    geom_density()
# Density plots of the prior_s samples   
...
# Use ggplot() to construct a density plot of the theta chain
ggplot(grade_chains, aes(x = sim, y = theta)) + geom_line()
# Use ggplot() to construct a density plot of the theta chain
ggplot(grade_chains, aes(x = theta)) + geom_density()
```
