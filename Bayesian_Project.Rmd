...
```
It is desirable to have stability in the trace plot (no patterns), with good mixing (ie.chain traverses the parameter space without getting stuck or missing a section). From our trace plots for Sigma and Theta we can see they have no patterns and there is good mixing, this indicates that they are stable. The density plots are also normally distributed. From the summary function we see that standard error of the mean of the MCMC chain is 0.004. It is desirable to have a high ratio of mean to SE and that is what we have. The mean grade is approximately between 54.8 and 58.5, Whereas the standard deviation density follows roughly the same uniform distribution as before
...
For every 1 unit of increase in Travel time, End of semester grade (G3) decreases by 1.35.
95% credible interval tells us that the drop can be from -2.34 to -0.38 per unit increase in freetime.
...
```
For every 1 unit of increase in studytime, End of semester grade (G3) increases by 3.65.
...
For every 1 unit of increase in studytime, End of semester grade (G3) increases by 3.55.
...
We have found that Model_2 is the best model.
...
From our models we derive that factor as Sex, StudyTime, Absences, Freetime, and Travel time have impact on students final grade.
We found out that Sex has not much impact on prediction grades model.
Difference in grades between sex turns out to be only 0.9 difference in mean grade between males and females.
From our presented models we found that variable FreeTime surprisingly has bigger impact on students fine year grade then variable such as Absences. 
For every unit of time of increasing free time students loose around 1 of their grade. Where in absence for every increasing unit decrease of grade is only 1/10 of it (0.11).
From this we can say that currant predicted grades model should not only consist of passed student grades from previous leaving cert year but also other factors such as they one we researched.
