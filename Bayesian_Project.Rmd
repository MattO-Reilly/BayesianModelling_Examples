...
grades_jags_mult <- jags.model(textConnection(grade_model_mult), 
                        ...
                        inits = list(.RNG.name="base::Wichmann-Hill",.RNG.seed = 1989),n.chains = 4)
...
```
It is desirable to have stability in the trace plot (no patterns), with good mixing (ie.chain traverses the parameter space without getting stuck or missing a section). Here the trace plots have good mixing for a, c, d and s. Whereas the standard deviation density follows roughly the same uniform distribution as before. It is desirable to have a high ratio of meean to SE and that is what we have.
...
```
For every 1 unit of increase in studytime, End of semester grade (G3) increases by 3.81.
For every 1 unit of increase in absences, End of semester grade (G3) decreases by 1.05.

#Studytime, Absences, freetime 
```{r}
# DEFINE the model    
grade_model_mult <- "model{
    # Define model for data Y[i]
...
grades_jags_mult <- jags.model(textConnection(grade_model_mult), 
                        data = list(Y = studentdata$G3,
                                    X2 = studentdata$studytime, X3 = studentdata$absences, X4 = studentdata$freetime), 
                        inits = list(.RNG.name="base::Wichmann-Hill",.RNG.seed = 1989),n.chains = 4)
...
# SIMULATE the posterior    
grades_sim_mult <- coda.samples(model=grades_jags_mult,
                                variable.names=c("a","c","d","e","s"),
                                n.iter=20000)

plot(grades_sim_mult[[1]][,1], density = TRUE, main = "a")
plot(grades_sim_mult[[1]][,2], density = TRUE, main = "c")
plot(grades_sim_mult[[1]][,3], density = TRUE, main = "d")
plot(grades_sim_mult[[1]][,4], density = TRUE, main = "e")
plot(grades_sim_mult[[1]][,5], density = TRUE, main = "s")
...
Model_2 <- dic.samples(model = grades_jags_mult,n.iter=1000)

```

For every 1 unit of increase in studytime, End of semester grade (G3) increases by 3.81.
For every 1 unit of increase in absences, End of semester grade (G3) decreases by 1.05.
For every 1 unit increase in freetime, 
...
#Absence and Free time
```{r}
freetime_model <- "model{
    # Likelihood model for Y[i]
    for(i in 1:length(Y)) {
      m[i] <- a + b * X[i]
      Y[i] ~ dnorm(m[i], s^(-2))
    ...
    b ~ dnorm(0, 19^(-2))
    s ~ dunif(0, 30)}"
freetime_jags <- jags.model(textConnection(freetime_model), 
                        data = list(Y = studentdata$G3, 
                                    X = studentdata$freetime), 
                        n.chains = 4)
# BURN IN the model
freetime_sim <- update(freetime_jags, n.iter = 10000)
# SIMULATE the posterior    
freetime_sim <- coda.samples(model=freetime_jags,
                         variable.names=c("a","b","s"),
                         n.iter=20000,
                         thin = 10)
```
...
plot(absence_sim[[1]][,2], main = 'b')
plot(absence_sim[[1]][,3], main = 's')
summary(freetime_sim)
```
The results are as suspected. The more time free form school has an effect of grades. From the mean we see a drop of 1 for every unit of free time.
95% credible interval tells us that the drop can be from -2.19 to -0.08 per unit increase in freetime.


#Grades and Travel time
```{r}
traveltime_model <- "model{
    # Likelihood model for Y[i]
    for(i in 1:length(Y)) {
      m[i] <- a + b * X[i]
      Y[i] ~ dnorm(m[i], s^(-2))
    ...
    b ~ dnorm(0, 19^(-2))
    s ~ dunif(0, 30)}"
traveltime_jags <- jags.model(textConnection(traveltime_model), 
                        data = list(Y = studentdata$G3, 
                                    X = studentdata$traveltime), 
                        n.chains = 4)
# BURN IN the model
traveltime_sim <- update(traveltime_jags, n.iter = 10000)
# SIMULATE the posterior    
traveltime_sim <- coda.samples(model=traveltime_jags,
                         variable.names=c("a","b","s"),
                         n.iter=20000,
                         thin = 10)
```
...
plot(absence_sim[[1]][,2], main = 'b')
plot(absence_sim[[1]][,3], main = 's')
summary(traveltime_sim)
```
Drop is 2.545 in grade for every unit time  unit time
and the 95% CI is -4.12 to -0.96.

```{r}
# DEFINE the model    
TravelandFreetime_model_mult <- "model{
    # Define model for data Y[i]
    for(i in 1:length(Y)) {
      Y[i] ~ dnorm(m[i], s^(-2))
      m[i] <- a + c*X2[i] + d*X3[i]
    ...
    # Define the a, b, c, d and s priors
    a ~ dnorm(0, 50^(-2))
    c ~ dnorm(-1, 2.11^(-2)) #free time
    d ~ dnorm(-2.545, 3.16^(-2)) #travel time
    s ~ dunif(0, 20)
}"  
```

```{r}
TravelandFreetime_jags_mult <- jags.model(textConnection(TravelandFreetime_model_mult), 
                        data = list(Y = studentdata$G3,
                                    X2 = studentdata$freetime, X3 = studentdata$traveltime), 
                        inits = list(.RNG.name="base::Wichmann-Hill",.RNG.seed = 1989))
# BURN IN the model
TravelandFreetime_sim_mult <- update(TravelandFreetime_jags_mult,n.iter=10000)
# SIMULATE the posterior    
TravelandFreetime_sim_mult <- coda.samples(model=TravelandFreetime_jags_mult,
                                variable.names=c("a","c","d","s"),
                                n.iter=20000)
plot(TravelandFreetime_sim_mult[[1]][,1], density = TRUE, main = "a")
plot(TravelandFreetime_sim_mult[[1]][,2], density = TRUE, main = "c")
plot(TravelandFreetime_sim_mult[[1]][,3], density = TRUE, main = "d")
plot(TravelandFreetime_sim_mult[[1]][,4], density = TRUE, main = "s")
```

```{r}
summary(TravelandFreetime_sim_mult)[1]
summary(TravelandFreetime_sim_mult)[2]
```
Chains are stable and dense. 
Chains A  abd C have small gaps but overallo they are well mix, C and D have almost no Gaps this indicates that the means we choose are well centered
The means and SE are have high ratios whitch is a good thing
avrg effect of freetime on grades is from -2.29 to -0.12 decrease in grades and 
avrg effect of travel time on grades is -4.23 to -1.17 decrease in grades



to compare this with our first found
free time: Drop is -2.19 to -0.08  per unit time
travel time; Drop is -4.12 to -0.96 per unit time

which confirms out chosen priors were accurate
